 ...
 */
function generate_activation_code($inst_id, $user_id, $db)
{
   $activation_code = uniqid();

   $datetime = date("Y-m-d H:i:s");

   $queryA="insert into INSTITUTION_MEMBER(user_id, institution_id, activation_code, datetime_created) values(?, ?, ?, ?)";
   $sqlA=$db->prepare($queryA);
   $sqlA->bind_param('iiss', $user_id, $inst_id, $activation_code, $datetime);
   $sqlA->execute();
   $sqlA->free_result();
...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
   ...
   if($inst_id != 0)
   {
      $activation_code = generate_activation_code($inst_id, $user_id, $db);
